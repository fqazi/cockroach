...
func TestLeaseManagerLockedTimestampCluster(t *testing.T) {
	...
	execConn.Exec(t, "SELECT * FROM d1.public.t1")
	moveToNextDescriptorVersion()
	// Wait for old versions to be purged before starting the txn, since
	// async purge (when the descriptor's modtime is ahead of the local
	// clock) may not have completed yet. If the txn begins before purge,
	// it can acquire an old version via locked timestamps, preventing
	// that version from ever being purged.
	assertDescriptorsCount(1)
